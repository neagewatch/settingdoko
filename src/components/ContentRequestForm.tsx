...
export default function ContentRequestForm({ query, os }: { query: string; os?: string }) {
  ...
  if (status === "done") return <p role="status" style={{ fontSize: 13, color: "#15803D", margin: "20px 0 0" }}>送信ありがとうございました。追加候補として確認します。</p>;
  return (
    <div className="content-request-form" style={{ maxWidth: 480, margin: "28px auto 0", padding: 16, border: "1px solid var(--border)", borderRadius: 10, background: "var(--surface)", textAlign: "left" }}>
      ...
      <button type="button" onClick={submit} disabled={status === "sending"} aria-busy={status === "sending"} style={{ marginTop: 8, padding: "8px 12px", border: 0, borderRadius: 7, background: "var(--primary)", color: "white", cursor: "pointer", fontWeight: 600, fontSize: 13 }}>{status === "sending" ? "送信中…" : "追加をリクエスト"}</button>
      {status === "error" && <p role="alert" style={{ color: "var(--danger)", fontSize: 12, margin: "8px 0 0" }}>送信できませんでした。管理者にお問い合わせください。</p>}
    </div>
  );
}
