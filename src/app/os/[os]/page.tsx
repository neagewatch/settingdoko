...
import { CATEGORIES, OSType, OS_LABELS, DEVICE_OS_TYPES, isAppPlatform } from "@/lib/types";
...
export default async function OSPage({ params, searchParams }: Props) {
  ...
  const appPlatform = osType !== "ipados" && isAppPlatform(osType);
  ...
  return (
    <div className="listing-page os-page" style={{ padding: "32px 0 60px" }}>
      ...
      <div className="listing-heading os-page-heading">
        <p className="section-index">{appPlatform ? "アプリ" : "端末"} / {OS_LABELS[osType]}</p>
      ...
      <section className="os-page-search" aria-labelledby="os-search-heading">
        <div className="os-page-search-heading">
          <span className="section-index">この{appPlatform ? "アプリ" : "端末"}から探す</span>
      ...
      <section className="os-results" aria-labelledby="os-results-heading">
        <div className="os-results-heading">
          <div>
            <p className="section-index">設定ガイド</p>
        ...
        {items.length > 0 ? (
          ...
          <div className="empty-state" style={{ padding: "40px 20px", textAlign: "center", border: "1px dashed var(--border)", color: "var(--text-muted)" }}>
            {osType === "ipados" ? (
              <>
                <p style={{ margin: 0, color: "var(--text)" }}>iPad / iPadOSの専用ガイドは準備中です。</p>
                <p style={{ margin: "8px 0 0", lineHeight: 1.7 }}>近い設定を探す場合は、検索またはiPhoneの設定一覧から確認してください。</p>
                <div style={{ display: "flex", justifyContent: "center", gap: 10, flexWrap: "wrap", marginTop: 16 }}>
                  <Link href="/search" className="secondary-button">検索する</Link>
                  <Link href="/os/ios" className="primary-button">iPhoneの設定を見る</Link>
                </div>
              </>
            ) : (
              <>
